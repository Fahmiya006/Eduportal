...
const saveStudents = (data) => fs.writeFileSync(FILE_PATH, JSON.stringify(data, null, 2));
...
const server = http.createServer((req, res) => {
  ...
  // POST new student
    ...
    req.on('end', () => {
      ...
      const newStudent = { ...JSON.parse(body), id: Date.now().toString() };
  ...
  // PUT update student
    ...
    req.on('end', () => {
      ...
      const updatedStudents = students.map(s =>
        String(s.id) === String(idToUpdate) ? { ...s, ...updatedData } : s
      );
  ...
  // DELETE student
    ...
    const updatedStudents = students.filter(s => String(s.id) !== String(idToDelete));
    saveStudents(updatedStudents);
    res.writeHead(200, { 'Content-Type': 'application/json' });
